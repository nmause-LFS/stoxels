 ...
 */
function _calcStepDurationMs(startPos, endPos, canvasW, canvasH) {
 ...
 */
function _walkAlongPath(sprite, points, segIdx, onComplete) {
    ...
    const durationMs = _calcStepDurationMs(startPos, endPos, canvasW, canvasH);

    _animateWalkStep(sprite, startPos, endPos, durationMs, () => {
        _walkAlongPath(sprite, points, segIdx + 1, onComplete);
    });
}
...
function _walkSpriteTo(targetWorldIdx, onArrived) {
    ...
    if (typeof _startAvatarWalkAnimation === 'function') {
        _startAvatarWalkAnimation('mv-sprite-img');
    }

    const points = _findWalkPath(_mvCurrentWorldIdx, targetWorldIdx);

    if (points.length < 2) {
        _onArriveAtWorld(sprite, walkingBar, targetWorldIdx, onArrived);
        return;
    }

    _walkAlongPath(sprite, points, 0, () => {
        _onArriveAtWorld(sprite, walkingBar, targetWorldIdx, onArrived);
    });
}
 ...
 */
function _onArriveAtWorld(sprite, walkingBar, targetWorldIdx, onArrived) {
    _mvWalking = false;
    ...
    _mvCurrentWorldIdx = targetWorldIdx;
    _saveSpritePosition(targetWorldIdx);
...
function _walkSpriteToHome() {
    ...
    const points = _findWalkPath(_mvCurrentWorldIdx, 'home');

    if (points.length < 2) {
        _onArriveAtHome(sprite);
        return;
    }
 ...
 * @param {HTMLElement} sprite
 */
function _onArriveAtHome(sprite) {
    _mvWalking = false;
    sprite.classList.remove('walking');
    ...
    _mvCurrentWorldIdx = null;
    _saveSpritePosition(null);
}
...
function showMapView() {
    // Restore the sprite's last known position from STATE
