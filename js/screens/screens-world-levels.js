...
const WD_WALK_SPEED_PX_PER_SEC = 100;
...
/**
 * Computes the animation duration (ms) for one walk segment based on the
 * canvas pixel distance and WD_WALK_SPEED_PX_PER_SEC.
 */
function _wdGetSegmentDurationMs(startPos, endPos, canvas) {
    const canvasW = canvas ? canvas.offsetWidth : 1000;
    const canvasH = canvas ? canvas.offsetHeight : 600;
    const dx = (endPos.x - startPos.x) / 100 * canvasW;
    const dy = (endPos.y - startPos.y) / 100 * canvasH;
...
/**
 * Animates one walk segment (points[segIdx] → points[segIdx + 1]) using
 * requestAnimationFrame, then calls walkNextSegment to continue the chain,
 * or calls onFinished when all segments are done.
 */
function _wdAnimateSegment(points, segIdx, onFinished, walkNextSegment) {
    const sprite = document.getElementById('wd-sprite');

    // All segments done — clean up and fire the callback
    if (segIdx >= points.length - 1) {
        sprite.classList.remove('walking');
        if (typeof _stopAvatarWalkAnimation === 'function') {
    ...
    const canvas = document.getElementById('wd-canvas');
    const durationMs = _wdGetSegmentDurationMs(startPos, endPos, canvas);
...
/**
 * Kicks off the full walk animation along a list of {x,y} waypoints.
 * Each consecutive pair of points is one segment. Calls onFinished when done.
 */
function _wdAnimateAlongPoints(points, onFinished) {
    const sprite = document.getElementById('wd-sprite');
    if (!sprite || points.length < 2) {
        if (onFinished) onFinished();
        return;
    }
    ...
    if (typeof _startAvatarWalkAnimation === 'function') {
        _startAvatarWalkAnimation('wd-sprite-img');
    }

    const walkNextSegment = (segIdx) =>
        _wdAnimateSegment(points, segIdx, onFinished, walkNextSegment);
    walkNextSegment(0);
}
...
/**
 * Walks the sprite to the target level node, persists its new position,
 * redraws paths, and fires onArrived once animation completes.
 */
function _wdWalkSpriteTo(wi, targetLevelIdx, onArrived) {
    if (_wdWalking) return;
    _wdWalking = true;

    const fromNode = _wdCurrentLevelIdx === null ? 'entrance' : _wdCurrentLevelIdx;
    const points = _wdFindWalkPath(wi, fromNode, targetLevelIdx);

    _wdAnimateAlongPoints(points, () => {
        _wdWalking = false;
...
/**
 * Walks the sprite back to the world entrance, persists position, and redraws paths.
 */
function _wdWalkSpriteToEntrance() {
    ...
    const fromNode = _wdCurrentLevelIdx === null ? 'entrance' : _wdCurrentLevelIdx;
    const points = _wdFindWalkPath(_wdCurrentWi, fromNode, 'entrance');

    _wdAnimateAlongPoints(points, () => {
        _wdWalking = false;
        _wdCurrentLevelIdx = null;
        _wdPersistSpritePos(_wdCurrentWi, null);
        _wdRefreshPaths(_wdCurrentWi);
    });
}
...
function showWorldDetail(wi) {
    ...
    // Restore the sprite's last known position, defaulting to the entrance
