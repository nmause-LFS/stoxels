...
function applyPenalty() {
    const penMult = getClassPenaltyMultiplier(); // class.js — 0 = absorbed by Variance Shield

    if (penMult === 0) {
        // Fully absorbed — no time cost, no mistake count, no visual feedback
        showToast(t('pen_shield')); // reuse the shield toast or add a separate key if you prefer
        return;
    }
