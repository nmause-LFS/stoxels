...
function _buildRowClueCell(row, clueIdx, padLeft, value, fontSize, colWidth) {
    ...
    return `<td class="rct rct-${row}" id="rct-${row}-${clueIdx}"` +
        ` style="font-size:${fontSize}px;position:sticky;left:${leftPx}px;background:var(--bg);z-index:10">` +
    ...
    for (let row = 0; row < rows; row++) {
        ...
        if (!isAdjMatrix) {
            ...
            for (let i = 0; i < padCells; i++) {
                html += `<td class="corner" style="position:sticky;left:${i * colWidth}px;background:var(--bg);z-index:11"></td>`;
            }
