...
function buildLS() {
    ...
    // ── Class indicator in top bar ─────────────────────────────────────────
    let classEl = document.getElementById('ls-class-indicator');
    if (!classEl) {
        classEl = document.createElement('div');
        classEl.id = 'ls-class-indicator';
        classEl.style.cssText = 'font-family:var(--PX);font-size:11px;padding:4px 10px;border:1px solid;margin-left:8px;white-space:nowrap;';
        document.getElementById('ls-score').insertAdjacentElement('afterend', classEl);
    }
    if (STATE.playerClass) {
        ...
        classEl.textContent = LANG === 'de' ? '⚗️ Absolviere ein Aufstiegs-Level um eine Klasse zu wählen' : '⚗️ Beat an Ascension level to choose a class';
        classEl.style.borderColor = 'var(--border2)';
        classEl.style.color = 'var(--accent2)';
    ...
    WORLDS.forEach((w, wi) => {
        ...
        w.data.forEach((p, li) => {
            ...
            const szStr = p.grid
                ? `${p.grid.length}×${p.grid[0].length}`   // actual rows × cols
            ...
            const ascensionBadge = isLastInWorld && w.data.length > 1
...
function _doStartLevel(gi) {
    ...
    const itemsNote = levelCells >= 200 && !STATE.done.includes(cur.gIdx)
        ...
        : '';
    bonusSidebarEl.textContent = (lvText(cur, 'bonusHint') || '');
